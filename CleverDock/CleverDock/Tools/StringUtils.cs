...
namespace CleverDock.Tools
{
    public class StringUtils
    {
        ...
        /// <param name="lowerLimit">Lower limit to start cutting words./param>
        ...
        public static string LimitCharacters(string input, int lowerLimit, int upperLimit)
        {
            if (input.Length < upperLimit)
                return input;
            string margin = input.Substring(lowerLimit, upperLimit - lowerLimit);
            if (margin.Contains(' '))
            {
                int lastSpace = margin.LastIndexOf(' ');
