...
public class WindowInterop
{
    ...
    ///     top-level window handles. The WNDENUMPROC type defines a pointer to this callback function. EnumWindowsProc is a
    ...
    /// </summary>
    public static IntPtr HWND_BOTTOM = 1;
    ...
    ///     A handle to a child window. The search begins with the next child window in the Z order. The
    ///     child window must be a direct child window of hwndParent, not just a descendant window.
    /// </param>
    ...
    /// <param name="bRepaint">
    ///     Indicates whether the window is to be repainted. If this parameter is TRUE, the window receives
    ...
    /// <returns>
    ///     If the window was brought to the foreground, the return value is nonzero. If the window was not brought to
    ///     the foreground, the return value is zero.
    /// </returns>
    [DllImport("user32.dll")]
    ...
    /// <param name="fForce">
    ///     A TRUE for this parameter will force the destruction of the window if an initial attempt fails to
    ///     gently close the window using WM_CLOSE. With a FALSE for this parameter, only the close with WM_CLOSE is attempted.
    /// </param>
    /// <returns>
    ///     If the function succeeds, the return value is nonzero. If the function fails, the return value is FALSE. To
    ///     get extended error information, call GetLastError.
    /// </returns>
    [DllImport("user32.dll")]
    ...
    ///     application-defined callback function. EnumThreadWindows continues until the last window is enumerated or the
    ///     callback function returns FALSE. To enumerate child windows of a particular window, use the EnumChildWindows
    ///     function.
    /// </summary>
    ...
    ///     style, the return value is nonzero. Otherwise, the return value is zero. Because the return value specifies whether
    ///     the window has the WS_VISIBLE style, it may be nonzero even if the window is totally obscured by other windows.
    /// </returns>
    [DllImport("user32.dll")]
    ...
    /// </param>
    /// <param name="dwFlags">
    ///     The type of animation. This parameter can be one or more of the following values. Note that, by
    ///     default, these flags take effect when showing a window. To take effect when hiding a window, use AW_HIDE and a
    ///     logical OR operator with the appropriate flags.
    /// </param>
    /// <returns></returns>
    [DllImport("user32.dll")]
    ...
    /// <param name="hWnd"> A handle to the window and, indirectly, the class to which the window belongs. </param>
    /// <param name="nIndex">
    ///     The zero-based offset to the value to be retrieved. Valid values are in the range zero through
    ///     the number of bytes of extra window memory, minus the size of an integer. To retrieve any other value, specify one
    ...
    public static extern IntPtr
        CallNextHookEx(IntPtr hhk, int nCode, WindowMessage wParam, [In] KBDLLHOOKSTRUCT lParam);
